...
## Entrada v12 — redemoinho NA lua (Jul/2026)

Pedido: "efeito na lua, tipo redemoinho, algo que justifique as cartas irem para ela". `shaders/boot_moon_vortex.glsl`: os PRÓPRIOS pixels do disco giram — cada raio na sua velocidade (falloff `smoothstep(0.10, 0.95, r)`: forte no miolo, ZERO na borda → emenda invisível), `ang = fall * strength * (0.55*t + 0.8)` com **t = splashTime (bounded!)** — getTime() enrolaria o miolo em ruído. `discard` fora do disco. Ordem: base → twinkle → **vórtice** → nuvens (passam na frente) → carga. Strength = `sealCharge*0.75 + 0.5*flare` (acorda devagar, agita com as cartas, surta no flash). Validado por screenshot: face da lua torcida diferente entre t=4 e t=7.5, borda/halo parados.
